...
# Ensure we can import the compatibility helpers from the repository layout
ROOT_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
PYTHON_DIR = os.path.join(ROOT_DIR, "python")
if PYTHON_DIR not in sys.path:
    sys.path.append(PYTHON_DIR)

from reactpyx import typed, type_param_compat

# Skip tests if Python version is below 3.13
pytestmark = pytest.mark.skipif(
    sys.version_info < (3, 13), reason="Python 3.13 specific features are being tested"
)
